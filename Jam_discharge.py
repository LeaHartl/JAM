...
ax[0].bar(merged.index, merged.Precip, width=1, color='k')
ax[0].set_ylabel('Daily sum [mm]')
merged_sum_mnth = merged['Precip'].resample('m').sum()
ax0 = ax[0].twinx()
ax0.step(merged_sum_mnth.index, merged_sum_mnth.values, color='grey')
ax0.set_ylabel('Monthly sum [mm]')
ax0.yaxis.label.set_color('grey')

ax[1].set_title('Snow height')
rol = merged.rolling(30, center=True).mean()
ax[1].plot(merged.index, merged.Snowdepth, label='daily mean', color='k', linewidth=0.5)
ax[1].plot(rol.index, rol.Snowdepth, label='30 day rolling mean', color='grey')
ax[1].legend()
ax[1].set_ylabel('[cm]')

ax[2].set_title('Discharge')
ax[2].plot(merged.index, merged.Stat1, color='k', linewidth=0.5, label='daily mean')
ax[2].plot(rol.index, rol.Stat1, color='grey', label='30 day rolling mean')
ax[2].set_ylabel('[$m^3/s$]')
